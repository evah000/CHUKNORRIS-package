...
chuk.path <- "E:/r package/CHUKNORRIS-master (tomato ver)/R/"  # *** MUST SPECIFY! ***
## Input folders - where is your data?
in1.path <- "E:/data/Spring 2025/3-4 34C VF36 heat/3-4 editing folder/"# *** MUST SPECIFY! ***
in2.path <- "E:/data/Spring 2025/3-4 34C VF36 heat/3-4 editing folder/"  # *** MUST SPECIFY! ***
## Output folder - where do you want your data saved?
out.path <- "E:/data/Spring 2025/3-4 34C VF36 heat/3-4 data folder/28 3/"  # *** MUST SPECIFY! ***
# Files to analyze
...
fl1.nm <- "28 3 RFP.txt"  # *** MUST SPECIFY! ***
## Specify channel name in kymograph 1
var1.nm <- "RFP"  # can basically be anything between ""
# File 2 MUST be the strongest channel, e.g. YFP
fl2.nm <- "28 3 GFP.txt"  # *** MUST SPECIFY! ***
## Specify variable name in kymograph 2
var2.nm <- "GFP"  # can basically be anything between ""
#-------------------------------------------------------------------------------
...
image.par <- list(
                  ...
                  # the tip estimate
                  tip.pixel = 15, # integer
                  # Number of pixels to measure Shank fluorescence counting from
                  # the tip estimate
                  shank.pixel = 96, # integer
                  # Number of pixels to extract fluorescence signal by averaging
                  # distributed symmetrically around the tip and shank region
                  avg.width = 29, # integer, usually an odd number
...
#-------------------------------------------------------------------------------
src.path <- paste(chuk.path, "src/", sep = "")
